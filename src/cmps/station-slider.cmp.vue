<template>
  <section>
    <swiper :options="swiperOptions" class="swiper container">
      <swiper-slide v-for="station in stations" v-bind:key="station.id">
        <section :style="setBgi" class="swiper-card ratio-card">
          <router-link :to="'/station/' + station._id">
            <section class="swiper-card-img-wrap ratio-square">
              <!-- <button @click.prevent="emitPlayStation(station._id)">{{playPause}}</button> -->
              <!-- <div class="swiper-overlay"></div> -->
            </section>
          </router-link> 
          <p class="swiper-title">{{station.title}}</p>
          <p class="swiper-description">{{station.description}}</p>
        </section>
      </swiper-slide>

      <div class="swiper-button-prev" slot="button-prev"></div>
      <div class="swiper-button-next" slot="button-next"></div>
    </swiper>
  </section>
</template> 

<script>
import { Swiper, SwiperSlide } from "vue-awesome-swiper";
import "swiper/css/swiper.css";

export default {
  name: "swiper-example-loop",
  title: "Loop mode / Infinite loop",
  props: {
    stations: Array,
    isPlaying: Boolean
  },
  data() {
    return {
      swiperOptions: {
        slidesPerView: 3,
        spaceBetween: 60,
        grabCursor: true,
        loop: true,
        navigation: {
          nextEl: ".swiper-button-next",
          prevEl: ".swiper-button-prev"
        },
        keyboard: {
          enabled: true,
          onlyInViewport: false
        }
      }
    };
  },
  computed: {
    setBgi() {
      // return `background-image: url('@/assets/imgs/station-default-thumb.png');`
      // return `background-image: url('data:image/png;base64,');`
    },
    // playPause() {
    //   return this.isPlaying ? "Pause" : "Play";
    // }
  },
  // methods: {
  //   emitPlayStation(stationId) {
  //     console.log('in slider');
  //     this.$emit("playStation", stationId);
  //     // if (this.isPlaying)  this.$emit("isPlaying", false);
  //     // if (!this.isPlaying)  this.$emit("isPlaying", true);
  //   }
     
  // },
  components: {
    Swiper,
    SwiperSlide
  }
};
</script>